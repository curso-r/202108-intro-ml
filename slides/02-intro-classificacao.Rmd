...
```{r}
Credit %>% slice(3:8) %>% select(Ethnicity) %>%
...
tab %>%
  select(-n, -p) %>%
  arrange(desc(Job)) %>%
  pivot_wider(names_from = Status, values_from = n_p) %>%
  relocate(good, .before = bad) %>%
  knitr::kable(format = "html")
```
...
tab %>%
  ...
  geom_text(aes(label = p), position = position_fill(vjust = 0.5), size = 6) +
  labs(x = NULL) +
  theme_minimal(25) +
  theme(legend.position = "bottom", axis.text.x = element_blank(), panel.grid = element_blank())
...
Casela de referência é **Job Fixed** e está representado pelo termo `(Intercept)`.


```{r, echo=FALSE}
credit_data <- credit_data %>% mutate(Status = factor(as.character(Status), levels = c("good", "bad")))
credit_data_mod <- glm(Status ~ Job, data = credit_data, family = "binomial")
```

```{r}
tab_estimativas <- broom::tidy(credit_data_mod)
tab_estimativas %>% 
  mutate(
    termo = term, 
    beta = round(estimate, 2),
    `exp(beta)` = round(exp(beta), 2),
    `logit(beta)` = round(exp(beta), 2),
    `exp(beta)` = ifelse(termo == "(Intercept)", "---", `logit(beta)`)
  ) %>%
  select(termo, beta) %>%
  # cbind(
  #   tab %>% filter(Status == "bad") %>% ungroup() %>% select(`prop obervada` = p)) %>%
  kableExtra::kable(format = "html")
```

$$\log\Bigg(\frac{p(x)}{1-p(x)}\Bigg) = -1.34 + 0.61 \text{freelance} + 0.93\text{others} +  1.75\text{partime}$$
...
```{r}
credit_data %>% slice(c(1,2,3,4,44,61,40,3949)) %>% select(Job) %>%
  cbind(model.matrix(~Job, data = .)) %>%
  knitr::kable(format = "html")
```

steps: `step_dummy(one_hot = TRUE)`
